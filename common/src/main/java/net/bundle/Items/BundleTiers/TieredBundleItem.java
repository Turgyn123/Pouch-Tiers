package net.bundle.Items.BundleTiers;
...
/**
 * A tiered bundle/bundle item. Each tier has a different capacity measured in "slots."
 * Internally this maps to the bundle weight system: N slots = weight limit of N
 * (since one full item stack = weight of 1).
 *
 * Slot counts:
 *   Base Bundle = 1, Copper = 2, Iron = 4, Gold = 8,
 ...
 */
public class TieredBundleItem extends BundleItem {

    /**
     * ThreadLocal used to communicate the current bundle's slot count
    ...
    public TieredBundleItem(Properties properties, int slotCount) {
        super(properties);
    ...
    /**
     * Right-click opens the bundle GUI instead of dumping contents (vanilla behavior).
     */
    @Override
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            ...
            player.openMenu(new SimpleMenuProvider(
                (syncId, inv, p) -> new BundleMenu(syncId, inv, slotIndex),
                stack.getHoverName()
            ));
        }
