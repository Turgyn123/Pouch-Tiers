...
/**
 * Renders the Bundle GUI with a dynamically-colored background
 * matching the bundle's dye color. All rendering is programmatic
 * (no texture PNGs needed). Uses the vanilla chest height formula
 * so player inventory aligns perfectly with the hotbar.
 *
 * Small bundles (≤8 slots) center their slots horizontally.
 * Large bundles (9+) fill 9-column rows like a standard chest.
 */
public class BundleScreen extends AbstractContainerScreen<BundleMenu> {
    ...
    public BundleScreen(BundleMenu menu, Inventory playerInv, Component title) {
        super(menu, playerInv, title);
        this.rows = menu.getContainerRows();
        this.cols = menu.getContainerCols();
        this.slotCount = menu.getSlotCount();
        this.baseColor = 0xFF000000 | menu.getBundleColor().getRgb();
    ...
    protected void renderBg(GuiGraphics g, float partialTick, int mouseX, int mouseY) {
        ...
        // ═══ Bundle section ═══
        int bundleH = rows * 18 + 19;

        // Outer border (3D raised panel)
        g.fill(x, y, x + imageWidth, y + bundleH, dark);
        g.fill(x, y, x + imageWidth - 1, y + bundleH - 1, light);
        g.fill(x + 2, y + 2, x + imageWidth - 2, y + bundleH - 2, medium);

        // Title area
        g.fill(x + 4, y + 4, x + imageWidth - 4, y + 14, baseColor);

        // Inner panel for slots
        g.fill(x + 4, y + 15, x + imageWidth - 4, y + bundleH - 3, baseColor);

        // Bundle slot backgrounds — each row individually centered
        ...
        int playerY = y + bundleH;
        int playerH = imageHeight - bundleH;
